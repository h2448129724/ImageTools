...
def validate_master_dataset(image_dir: str | Path, annotation_dir: str | Path) -> dict:
    ...
    for stem in sorted(set(image_map) & set(json_map)):
        ...
        try:
            ...
            sample_reports.append({"sample_id": stem, "json_path": str(json_path), "errors": [f"JSON 读取失败: {exc}"], "warnings": []})
        ...
        try:
            ...
            sample_reports.append({"sample_id": stem, "json_path": str(json_path), "errors": [f"图片读取失败: {exc}"], "warnings": []})
...
def export_master_to_model_a(
    ...
    output_annotation_dir: str | Path,
    *,
    ...
    for stem, image_path in sorted(image_map.items()):
        ...
        master = _load_master_from_pair(image_path, json_map.get(stem))
